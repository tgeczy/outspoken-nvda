...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def _askCombined(self):
        ...
        try:
            ...
            log.error("outSPOKEN: combined dialog failed", exc_info=True)
    ...
    def _ask(self, missing):
        ...
        """
        try:
            answer = gui.messageBox(_combined_message(missing), _DIALOG_TITLE,
                                    wx.YES_NO | wx.CANCEL | wx.ICON_INFORMATION)
            if answer == wx.YES:
                os.startfile(_folder_to_open(missing))
            elif answer == wx.NO:
                # "Do not ask again" was answered about all of them, so it is
                # recorded for all of them. Asking three times about a question
                # already declined is the thing being fixed.
                for m in missing:
                    try:
                        with open(os.path.join(m["folder"], _MARKER), "w") as f:
                            f.write("Delete this file to be asked about the "
                                    "engine again.\n")
                    except OSError:
                        log.error("outSPOKEN: could not write %s in %s"
