...
class FakeWavePlayer(object):
    ...
    exactly this, so the fake has to keep the real one's promises.  Copied
    from Panthera's, which learned them first.
    """
...
def _install_fake_nvda():
    ...
    class CharacterModeCommand(object):
        """NVDA wraps single-character strings in these when spelling."""
