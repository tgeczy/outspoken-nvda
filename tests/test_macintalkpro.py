...
# -- the pipeline, and reuse: tests that build a SECOND osp.Host ------------
#
# These come LAST on purpose. Each builds its own `osp.Host`, and there is one
# emulator behind the DLL: a second `osp_init` re-initialises the shared RAM --
# heap primed like a real Mac's, per the allocator note in osp_host_files.c --
# which wedges the module-scoped `pro` engine every later render would use. So
# every test that renders through `pro` runs above this line, on an engine no
# second host has touched. (`live()` cannot catch this: the wedged engine is
# not `_dead`, it just renders silence -- see tests/inflectioncheck.py.)
