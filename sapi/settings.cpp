...
namespace outspoken_sapi {
/* The settings the NVDA driver has and SAPI users were living without --
 * inflection and how numbers are read -- plus the diagnostic log level and
 * the read timeout, kept by the settings program and read afresh on every
 * Speak, so a change takes effect on the very next thing spoken.  The two
 * the *host* reads rather than this code -- inflection and numbers -- take
 * effect by replacing the host; see host_ensure.
 *
 * **They live in a file, and the registry is the fallback.**  Panthera's
 * model from its 3.1.0, carried over whole.  Two files, one typed value at a
 * time: this user's, under %APPDATA%, and the machine's, under %ProgramData%.
 * A person's own choice has to win, and it also has to be *possible*: a
 * voice speaking under a service account -- the Windows sign-in screen is
 * the one that matters -- has an %APPDATA% belonging to that account and
 * holding nothing anybody chose.  %ProgramData% is readable by every account
 * and writable by a standard user without elevation, so the settings program
 * can keep the machine copy current on every save, and the sign-in screen
 * speaks with the settings its owner chose last.  It also ends the WOW64
 * dance: both builds of this DLL read the same two files.
 *
 * The registry keeps its place behind the files so nothing breaks on the day
 * of the upgrade: HKCU then HKLM, per value, as 1.2.x read Diagnostics and
 * ReadTimeoutMs, until the settings program has moved a person's values out.
 * DataPath is not a setting and never comes through here; every voice token
 * carries its own.
 *
...
std::wstring settings_serialize(const SettingsTable &table) {
    std::wstring out = L"# outSPOKEN SAPI settings.  Written by the settings program; safe to edit by hand.\r\n"
...
std::wstring settings_user_path() {
    return settings_path(FOLDERID_RoamingAppData, L"APPDATA", L"OUTSPOKEN_SAPI_SETTINGS_USER");
}
std::wstring settings_machine_path() {
    return settings_path(FOLDERID_ProgramData, L"ProgramData", L"OUTSPOKEN_SAPI_SETTINGS_MACHINE");
}

/* ---- the cache ---------------------------------------------------------- */
/*
 * A few values are read per utterance.  Each file is stat'ed on every
