 ...
 * lookup.  Built and run by build.ps1, linking the shipping settings.cpp.
 * Both files are pointed at scratch paths through the environment and the
 * registry is redirected, so nobody's settings are read or written.
 * Panthera's test, carried over with the names changed. */
#include "settings.h"
#include <cstdio>
#include <string>
using namespace outspoken_sapi;
...
/* Redirect only this process's registry access. */
class TestRegistry {
    HKEY user, machine;
    std::wstring path;
public:
    ...
    TestRegistry():user(0),machine(0),ready(false) {
        wchar_t suffix[80];
        swprintf_s(suffix,L"Software\\OutspokenTests\\Settings-%lu",GetCurrentProcessId());
        path=suffix;
...
int wmain() {
    ...
    wchar_t leaf[64]; swprintf_s(leaf,L"outspoken-settings-test-%lu",GetCurrentProcessId());
    ...
    SetEnvironmentVariableW(L"OUTSPOKEN_SAPI_SETTINGS_USER",user.c_str());
    SetEnvironmentVariableW(L"OUTSPOKEN_SAPI_SETTINGS_MACHINE",machine.c_str());
    TestRegistry reg;
    ...
    check(setting_string(L"NumberStyle",L"words")==L"digits","a value the user file lacks comes from the machine file");
    reg_string(HKEY_CURRENT_USER,L"Diagnostics",L"loud");
    reg_dword(HKEY_CURRENT_USER,L"ReadTimeoutMs",4000);
    check(setting_dword(L"ReadTimeoutMs",30000)==4000,"a value neither file has comes from the registry");
    ...
    written.push_back(std::make_pair(std::wstring(L"NumberStyle"),str(L"words")));
    check(settings_write_file(user,written),"the writer puts a file on disk");
    check(setting_dword(L"Inflection",50)==40&&setting_string(L"NumberStyle",L"digits")==L"words",
